...
- **Runtime Call Tracing**: A dynamic tracer runs your code (typically the test suite) and merges the calls that actually happened into the graph as `CALLS` edges — flagged where static analysis missed them — so dispatch through interfaces, virtual methods, function pointers, reflection, and framework routing lands with true invocation counts. Convert a run from Python, the JVM, Node.js, .NET, PHP, Lua, Dart, Go, or C/C++ with `cgr trace`.
