# (H) Multi-language retrieval (Lua). Extends the file-level call-localization
# (H) benchmark to Lua: for each first-party Lua function, which files call it.
# (H) cgr's Lua CALLS edges (reduced to (caller_file, callee_simple_name)) are
# (H) graded against call sites extracted by luaparse, over the same first-party
# (H) name universe. luaparse is independent of cgr's tree-sitter Lua frontend,
# (H) so this measures cgr's cross-file Lua call resolution against ground truth
# (H) (mirrors evals/php_retrieval.py / java_retrieval.py / ts_retrieval.py).
