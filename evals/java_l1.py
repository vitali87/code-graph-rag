...
from .cgr_graph import extract_cgr_java_nodes
...
def main(
    ...
    cgr = GraphData(
        nodes=extract_cgr_java_nodes(target, project), edges=set(), name_edges=set()
    )
    ...
    oracle = GraphData(nodes=run_java_oracle(target), edges=set(), name_edges=set())
