...
| `glosses` | Notes (Glosses) about one definition: `annotating` are filed on it, `mentioning` are filed on other definitions and refer to it. `target` is resolved as `annotate` resolves it. Each note's `anchor_state` is graded after each sync for function and method subjects: EXACT when the definition's current content hash equals the one recorded when the note was written, STALE when it differs (and EXACT again if the code is reverted). A note on a class or module, or one written before hashes were recorded, keeps the state it has. Deterministic: fixed graph queries, no LLM, same graph gives the same JSON. Use this instead of query_code_graph whenever you know the exact name or location.  |
