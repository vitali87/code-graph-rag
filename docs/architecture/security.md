...
**The analysed repository is untrusted input, with one important exception.** Most language frontends are pure Tree-sitter parsers: they read bytes and produce syntax trees, so a hostile repository can at worst produce a wrong graph, not code execution. Two frontends go further. The C++ frontend defaults to `CPP_FRONTEND=hybrid`, which additionally parses translation units with libclang when the [`cpp` extra and a compilation database](../guide/cpp-semantic-mode.md) are available; this is still parsing only. The C# frontend defaults to `CSHARP_FRONTEND=treesitter`, which parses only. Its opt-in modes do more: `auto` resolves to the Roslyn-based hybrid frontend whenever a `dotnet` toolchain is on `PATH`, and `hybrid`/`roslyn` force it. That path runs `dotnet restore` and evaluates the project's MSBuild files, and source generators from the analysed repository execute inside the compilation, so with the .NET SDK installed those modes run parts of the analysed repository's build with your privileges. Choose them only for repositories whose build you trust; the shipped default does not. When they do run, the .NET CLI is invoked with `DOTNET_CLI_TELEMETRY_OPTOUT=1`, so the toolchain itself reports nothing. See the [graph schema documentation](graph-schema.md) for the frontend matrix.
...
**The LLM, embedding and web-search boundaries.** Code or queries can leave the machine on three paths. Two are chosen by configuration: interactive querying sends snippets and questions to the configured model provider, and semantic indexing sends code to the embedding provider when `CGR_EMBEDDING_PROVIDER=openai` is set. With a local model provider (Ollama) and the default local UniXcoder embeddings, neither leaves the machine.

The third is the CLI agent's `research` tool, and unlike the other two it needs no configuration to be active. The search backend registers unconditionally: keyless DuckDuckGo by default (no account required), with `WEB_SEARCH_PROVIDER=serpdive` plus `SERPDIVE_API_KEY` as an opt-in alternative. Only the search query is sent, never repository contents wholesale, and an egress gate narrows that further: tools returning repository bytes to the model record what they returned, and the search tool refuses any query carrying a verbatim span of it ([#1128](https://github.com/vitali87/code-graph-rag/issues/1128)). That check is a deliberately cheap string match rather than semantic taint tracking, so PARAPHRASED repository content still passes; the enforceable boundary is the sub-agent below, and this closes the direct channel of the orchestrator pasting code into a query. Web search is also deliberately not an orchestrator tool: it lives alone in a leaf research sub-agent whose summary crosses back as data, so fetched web content never shares a context with repository reads and shell. The MCP server exposes neither `research` nor `web_search`, deliberately: it has no equivalent egress gate, so the boundary is kept closed there rather than watched. To keep a CLI session entirely offline, do not invoke `research`; network egress otherwise follows the model and embedding providers above.

All three use TLS with standard certificate verification (httpx defaults). API keys are supplied via environment variables or `.env`, are excluded from version control, and are never written to the graph.
