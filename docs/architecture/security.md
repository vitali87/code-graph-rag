...
**The graph and vector stores are local processes bound to loopback by default.** Memgraph and Qdrant run in local Docker containers managed by `cgr daemon`. Every published port (7687, 7444, 3000, 6333, 6334) is bound to `127.0.0.1` unless you say otherwise, so other hosts on the network cannot reach them. This closes the drive-by exposure reported in issue [#1012](https://github.com/vitali87/code-graph-rag/issues/1012), the same exposure class as [#808](https://github.com/vitali87/code-graph-rag/issues/808) for the MCP HTTP server. Setting `CGR_STACK_BIND_HOST` widens the bind (for example to `0.0.0.0` to reach the stack from another machine), and that is a deliberate risk: Memgraph Bolt, Memgraph Lab, and Qdrant are all UNAUTHENTICATED, so a wider bind puts the whole code graph on the network with no credential in front of it. Treat the graph with the same confidentiality as the code itself.
