...
| `WEB_SEARCH_PROVIDER` | `duckduckgo` | Web-search backend: keyless `duckduckgo`, `serpdive` or `serply`. The web-search tool is registered by default, so searches can send query text to the selected external provider even when this variable is unset. |
...
| `SERPLY_API_KEY` | _(unset)_ | API key used when `WEB_SEARCH_PROVIDER=serply`, which returns Google's results through [Serply](https://serply.io) (free tier, no card); without one, web search falls back to DuckDuckGo. |
