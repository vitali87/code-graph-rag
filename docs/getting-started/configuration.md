...
| `CSHARP_FRONTEND` | `treesitter` | C# frontend mode: `treesitter`, `auto`, `hybrid`, or `roslyn`. The non-default Roslyn-backed modes require `dotnet` and run `dotnet restore`, which evaluates the analysed repository's MSBuild files and source generators. Use them only for repositories you trust. See the [graph schema documentation](../architecture/graph-schema.md). |
