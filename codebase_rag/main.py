...
"""
    ...
    while True:
        try:
            ...
            if confirm_edits:
                console.print("\n[bold yellow]⚠️  This optimization will modify files in your codebase.[/bold yellow]")
                
                if not Confirm.ask("[bold cyan]Do you want to proceed with these optimizations?[/bold cyan]"):
                    console.print("[bold red]❌ Optimizations rejected by user.[/bold red]")
                    
                    # Ask the agent to acknowledge the rejection
                    rejection_message = "The user has rejected the proposed optimizations. Please acknowledge this and suggest alternative approaches or ask what specific changes they would prefer."
                    ...
                    # Ask the agent to proceed with the approved changes
                    approval_message = "The user has approved the proposed optimizations. Please proceed with implementing them."
                    
                    with console.status("[bold green]Implementing approved optimizations...[/bold green]"):
                        approval_response = await run_with_cancellation(
                            console,
                            rag_agent.run(approval_message, message_history=message_history),
                        )
                    
                    if not (isinstance(approval_response, dict) and approval_response.get("cancelled")):
                        approval_markdown = Markdown(approval_response.output)
                        console.print(
                            Panel(
                                approval_markdown,
                                title="[bold green]Implementation Result[/bold green]",
                                border_style="green",
                            )
                        )
                        message_history.extend(approval_response.new_messages())
...
async def run_chat_loop(
    ...
    while True:
        try:
            ...
            # Check if confirmation is needed for edit operations
            if confirm_edits :
                console.print("\n[bold yellow]⚠️  This operation will modify files in your codebase.[/bold yellow]")
                
                if not Confirm.ask("[bold cyan]Do you want to proceed with these changes?[/bold cyan]"):
                    console.print("[bold red]❌ Changes rejected by user.[/bold red]")
                    
                    # Ask the agent to acknowledge the rejection
                    rejection_message = "The user has rejected the proposed changes. Please acknowledge this and ask if they would like to try a different approach or modify the changes."
                    ...
                    console.print("[bold green]✅ Changes approved by user.[/bold green]")
                    markdown_response = Markdown(response.output)
                    console.print(
                    Panel(
                        markdown_response,
                    ...
                    # Ask the agent to proceed with the approved changes
                    approval_message = "The user has approved the proposed changes. Please proceed with implementing them."
                    
                    with console.status("[bold green]Implementing approved changes...[/bold green]"):
                        approval_response = await run_with_cancellation(
                            console,
                            rag_agent.run(approval_message, message_history=message_history),
                        )
...
async def main_async(repo_path: str) -> None:
    ...
    table.add_row("Edit Confirmation", "Enabled" if confirm_edits else "Disabled")
...
def start(
    ...
    global confirm_edits
    
    # Set confirmation mode based on flag
    confirm_edits = not no_confirm
...
"""
    ...
    while True:
        try:
            ...
            if "[y/n]" in question or pending_confirmation:
            ...
            # Format response with confirmation request if it contains edit operations
            formatted_output = format_confirmation_request(response.output)
            
            # Check if we need confirmation for the next iteration
            if "[y/n]" in formatted_output and confirm_edits:
                pending_confirmation = True
            
            # Store the formatted output for next iteration
            question = formatted_output
            
            markdown_response = Markdown(formatted_output)
            console.print(
                Panel(
                    markdown_response,
                    title="[bold green]Optimization Agent[/bold green]",
