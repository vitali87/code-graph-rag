...
"""
    ...
    assert len(found_functions) >= 4, (
        f"Expected at least 4 lambda functions, found {len(found_functions)}: {found_functions}"
    )
...
"""
    ...
    print("✅ C++ lambdas and functional programming relationship validation passed:")
    print(f"   - CALLS relationships: {len(call_relationships)}")
    print(f"   - DEFINES relationships: {len(defines_relationships)}")
    print(f"   - Comprehensive lambda calls: {len(comprehensive_calls)}")
