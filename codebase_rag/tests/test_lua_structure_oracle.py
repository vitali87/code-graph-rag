# (H) Covers the Lua structure oracle harness (evals/oracles/lua_oracle +
# (H) evals/lua_l1.py): the luaparse oracle is authoritative ground truth, and
# (H) cgr's captured Lua nodes are graded against it on (kind, file, start_line).
# (H) Lua has no classes, so every function is a Function.
...
def test_cgr_matches_luaparse_oracle_on_lua_structure(tmp_path: Path) -> None:
    ...
    oracle = GraphData(nodes=run_lua_oracle(project), edges=set(), name_edges=set())
