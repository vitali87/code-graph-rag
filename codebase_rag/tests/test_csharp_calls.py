...
def test_verbatim_identifier_constructor_call_resolves(
    csharp_project: Path, mock_ingestor: MagicMock
) -> None:
    (csharp_project / "VerbatimCtor.cs").write_text(
        """
namespace N;
public class @event {
    public @event() { }
...
""",
    ...
    targets = _call_targets(mock_ingestor)
    assert any(t.endswith("N.@event.@event") for t in targets), targets
