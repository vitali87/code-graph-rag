...
"""
    ...
    assert len(found_functions) >= 3, (
        f"Expected at least 3 macro-related functions, found {len(found_functions)}: {found_functions}"
    )
...
"""
    ...
    assert len(found_functions) >= 2, (
        f"Expected at least 2 conditional compilation functions, found {len(found_functions)}: {found_functions}"
    )
...
"""
    ...
    found_functions = [func for func in expected_functions if func in created_functions]
    assert len(found_functions) >= 2, (
        f"Expected at least 2 pragma directive functions, found {len(found_functions)}: {found_functions}"
    )
...
"""
    ...
    print("✅ C++ preprocessor relationship validation passed:")
    print(f"   - CALLS relationships: {len(call_relationships)}")
    print(f"   - DEFINES relationships: {len(defines_relationships)}")
    print(f"   - Comprehensive preprocessor calls: {len(comprehensive_calls)}")
