...
"""
    ...
    assert len(found_classes) >= 2, (
        f"Expected at least 2 constexpr classes, found {len(found_classes)}: {found_classes}"
    )
    ...
    assert len(found_functions) >= 4, (
        f"Expected at least 4 constexpr functions, found {len(found_functions)}: {found_functions}"
    )
