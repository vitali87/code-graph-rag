...
"""
    ...
    assert len(found_functions) >= 3, (
        f"Expected at least 3 designated initializer functions, found {len(found_functions)}: {found_functions}"
    )
    ...
    assert len(found_classes) >= 2, (
        f"Expected at least 2 designated initializer classes, found {len(found_classes)}: {found_classes}"
    )
...
"""
    ...
    assert len(found_functions) >= 4, (
        f"Expected at least 4 consteval functions, found {len(found_functions)}: {found_functions}"
    )
