...
@pytest.mark.asyncio
async def test_change_signature_refuses_on_an_incomplete_graph() -> None:
    """`_graph_query` refuses only for tools named in `_READS_THE_GRAPH`.

    `change_signature` routed through the dispatcher from the day it was
    added but was never listed there, so the `if tool in _READS_THE_GRAPH`
    condition was False and the refusal was skipped: it rewrote signatures
    from a partial graph, whose missing call sites read as "no other
    callers". Membership alone is what turns the guard on, so this asserts
    the BEHAVIOUR rather than the set.
    """
    from unittest.mock import MagicMock
    ...
    handler._graph_incomplete = True
    handler._persisted_incomplete = MagicMock(return_value=True)
    handler._run_change_signature = MagicMock()

    result = await handler.change_signature("pkg.mod.fn", ["a"], project=ALPHA)

    # The rewrite must not run. Asserting only on the payload would pass for
    # a signature change that happened and then reported an error.
    handler._run_change_signature.assert_not_called()
    assert ALPHA in result[cs.DICT_KEY_ERROR]


@pytest.mark.asyncio
async def test_change_signature_runs_when_the_graph_is_whole() -> None:
    """The control: a guard that refused unconditionally would pass above."""
    from unittest.mock import MagicMock
    ...
    handler._graph_incomplete = False
    handler._persisted_incomplete = MagicMock(return_value=False)
    handler._run_change_signature = MagicMock(return_value={"ok": True})

    result = await handler.change_signature("pkg.mod.fn", ["a"], project=ALPHA)

    handler._run_change_signature.assert_called_once()
    assert result == {"ok": True}
