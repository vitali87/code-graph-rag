...
"""
    ...
    assert len(found_classes) >= 3, (
        f"Expected at least 3 friend function classes, found {len(found_classes)}: {found_classes}"
    )
    ...
    assert len(found_functions) >= 2, (
        f"Expected at least 2 friend function test functions, found {len(found_functions)}: {found_functions}"
    )
