...
def _path_order_terms(query: str) -> list[str]:
    """The ORDER BY terms that sort on a path property."""
    order_at = query.find("ORDER BY")
    if order_at == -1:
        return []
    clause = query[order_at + len("ORDER BY") :].splitlines()[0]
...
def test_the_order_key_agrees_with_the_projection() -> None:
    ...
    assert terms in ([], [projection]), (terms, projection)
