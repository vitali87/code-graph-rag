...
def test_javascript_direct_io_sinks(
    ...
    _build(graph_ingestor, tmp_path, "app.js", _JS_CODE)
...
def test_typescript_direct_io_sinks(
    ...
    _build(graph_ingestor, tmp_path, "app.ts", _JS_CODE)
    edges = _io_edges(graph_ingestor)
    assert (_WRITES, "resource::STDOUT::<dynamic>") in edges
    assert (_READS, "resource::NETWORK::https://api.example.com/data") in edges
    assert (_WRITES, "resource::FILE::out.txt") in edges
...
def test_shadowed_local_import_emits_no_edge(
    ...
    assert (_WRITES, "resource::FILE::a.txt") not in _io_edges(graph_ingestor)
...
def test_real_builtin_import_still_emits(
    ...
    assert (_WRITES, "resource::FILE::a.txt") in _io_edges(graph_ingestor)
...
def test_destructured_local_shadows_sink(
    ...
    assert (_READS, "resource::NETWORK::https://n/a") not in _io_edges(graph_ingestor)
...
def test_destructured_param_shadows_sink(
    ...
    assert (_READS, "resource::NETWORK::https://n/b") not in _io_edges(graph_ingestor)
...
def test_block_scoped_local_does_not_over_shadow(
    ...
    assert (_WRITES, "resource::FILE::out.txt") in _io_edges(graph_ingestor)
...
def test_block_local_shadows_within_its_block(
    ...
    assert (_WRITES, "resource::FILE::out.txt") not in _io_edges(graph_ingestor)
...
def test_esm_named_import_emits(graph_ingestor: GraphIngestor, tmp_path: Path) -> None:
