...
def test_rust_direct_io_sinks(graph_ingestor: GraphIngestor, tmp_path: Path) -> None:
    ...
    _build(graph_ingestor, tmp_path, _RUST_CODE)
...
def test_rust_nested_closure_not_credited(
    ...
    assert (_WRITES, "resource::FILE::secret.txt") not in _io_edges(graph_ingestor)
