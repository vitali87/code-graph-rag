...
def test_java_direct_io_sinks(graph_ingestor: GraphIngestor, tmp_path: Path) -> None:
    ...
    _build(graph_ingestor, tmp_path, _JAVA_CODE)
...
def test_java_files_read(graph_ingestor: GraphIngestor, tmp_path: Path) -> None:
...
def test_java_local_shadows_system(
    ...
    assert (_READS, "resource::ENV::SECRET") not in _io_edges(graph_ingestor)
...
def test_java_imported_files_emits(
    ...
    assert any(
        ...
        for rel, qn in _io_edges(graph_ingestor)
    )
...
def test_java_earlier_declarator_init_not_shadowed(
    ...
    assert (_READS, "resource::ENV::A") in _io_edges(graph_ingestor)
...
def test_java_foreach_iterable_call_not_shadowed(
    ...
    assert (_READS, "resource::ENV::PATH") in _io_edges(graph_ingestor)
...
def test_java_foreach_loopvar_shadows_in_body(
    ...
    assert (_READS, "resource::ENV::X") not in _io_edges(graph_ingestor)
