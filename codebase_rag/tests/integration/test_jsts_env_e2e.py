...
def _reads(ingestor: GraphIngestor) -> set[str]:
...
def test_process_env_member_and_subscript_reads(
    ...
    reads = _reads(graph_ingestor)
    assert "resource::ENV::SECRET" in reads
    assert "resource::ENV::TOKEN" in reads
    assert "resource::ENV::URL" in reads


def test_process_env_typescript(graph_ingestor: GraphIngestor, tmp_path: Path) -> None:
    _build(
        graph_ingestor,
        tmp_path,
        "app.ts",
    ...
    assert "resource::ENV::HOME" in _reads(graph_ingestor)
...
def test_shadowed_process_emits_no_env_read(
    ...
    assert "resource::ENV::SECRET" not in _reads(graph_ingestor)
