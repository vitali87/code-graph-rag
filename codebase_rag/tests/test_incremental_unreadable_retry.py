...
def test_a_path_under_a_directory_now_a_file_is_gone(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """`pkg/module.py` after `pkg/` was replaced by a regular file: `lstat`
    raises ENOTDIR (POSIX), and the path can never exist again. Reading it
    as unreachable kept its subtree through every orphan prune (bot review
    on PR #1993). A permission failure on the same lookup stays unreachable."""
    (tmp_path / "pkg").write_text("not a directory\n")
    under_a_file = tmp_path / "pkg" / "module.py"
    assert gu._vanished(under_a_file) is True
    ...
    def denied(path, *args, **kwargs):  # noqa: ANN001, ANN002, ANN003, ANN202
        if Path(str(path)) == present:
            raise PermissionError(errno.EACCES, "Permission denied", str(path))
        return real_lstat(path, *args, **kwargs)

    monkeypatch.setattr(gu.os, "lstat", denied)
    assert gu._vanished(present) is False
...
def test_the_orphan_prune_does_not_raise_on_an_unreachable_link(
    ...
    Whether `exists()` itself raises depends on the interpreter (3.14 reads
    every OSError as False), so the precondition is checked with `os.stat`,
    which raises on every version when the target really is unreachable.
    """
    ...
    try:
        # The precondition: the target really is unreachable, so the
        # assertions below are not passing on a fixture that never had the
        # problem. Root, and Windows ACLs, ignore the mode bits.
        try:
            os.stat(link)
        except PermissionError:
            pass
        else:
            pytest.skip("the mode bits do not block access on this host")
