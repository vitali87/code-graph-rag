# (H) A TS arrow bound through a CAST wrapper (`export const createStore = ((s) =>
# (H) s ? createStoreImpl(s) : createStoreImpl) as CreateStore`, zustand's public-API
# (H) shape) lost ALL its body's call/reference edges: `_js_ts_arrow_binding_name`
# (H) required the arrow to be the declarator's DIRECT value, so the cast/paren
# (H) wrapper made it "anonymous", its calls bubbled to module scope, and dropped.
# (H) The def pass already unwraps the cast (registers `createStore`); the call pass
# (H) must agree, so the impl and its inner functions stay reachable.
...
def test_double_cast_wrapped_function_expression_binding(
    ...
    # (H) Nested casts (`as unknown as T`) and a function expression instead of an
    # (H) arrow: the unwrap must loop through every wrapper layer.
