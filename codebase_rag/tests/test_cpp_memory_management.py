...
"""
    ...
    assert len(found_classes) >= 4, (
        f"Expected at least 4 smart pointer classes, found {len(found_classes)}: {found_classes}"
    )
...
"""
    ...
    assert len(found_classes) >= 2, (
        f"Expected at least 2 move semantics classes, found {len(found_classes)}: {found_classes}"
    )
