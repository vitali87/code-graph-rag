...
# Pytest Fixtures for setting up test environments and mocks
...
@pytest.fixture
def mock_updater(temp_repo: Path) -> MagicMock:
    ...
    mock.ingestor = MagicMock()
    mock.parsers = {"python": MagicMock()}
    mock.queries = {"python": {"config": MagicMock()}}
    return mock
...
def test_file_creation_flow(event_handler: CodeChangeEventHandler, mock_updater: MagicMock, temp_repo: Path) -> None:
...
def test_file_modification_flow(event_handler: CodeChangeEventHandler, mock_updater: MagicMock, temp_repo: Path) -> None:
...
def test_file_deletion_flow(event_handler: CodeChangeEventHandler, mock_updater: MagicMock, temp_repo: Path) -> None:
...
def test_irrelevant_files_are_ignored(event_handler: CodeChangeEventHandler, mock_updater: MagicMock, temp_repo: Path) -> None:
...
def test_directory_creation_is_ignored(event_handler: CodeChangeEventHandler, mock_updater: MagicMock, temp_repo: Path) -> None:
...
def test_unsupported_file_types_are_ignored(event_handler: CodeChangeEventHandler, mock_updater: MagicMock, temp_repo: Path) -> None:
