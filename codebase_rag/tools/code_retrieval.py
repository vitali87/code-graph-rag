...
class CodeRetriever:
    ...
    def _parse_file_and_find_node(self, file_path: Path, 
                                 qualified_name: str) -> Optional[Tuple[ast.AST, List[str]]]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if self._node_matches_qualified_name(node, file_path, 
                                                   qualified_name, tree):
    ...
    def _node_matches_qualified_name(self, node: ast.AST, file_path: Path,
                                   qualified_name: str, tree: ast.AST) -> bool:
    ...
    def _get_qualified_name_for_node(self, node: ast.AST, file_path: Path, 
        ...
        if isinstance(node, ast.ClassDef):
            ...
            if class_node:
                return '.'.join([project_name] + module_parts + [class_node.name, node.name])
            else:
                return '.'.join([project_name] + module_parts + [node.name])
        
        return None
    
    def _find_parent_class(self, target_node: ast.AST, 
                          tree: ast.AST) -> Optional[ast.ClassDef]:
    ...
    def _extract_node_info(self, node: ast.AST, 
                          source_lines: List[str]) -> Dict[str, Any]:
    ...
    async def find_code_snippet(self, qualified_name: str) -> CodeSnippet:
        ...
        try:
            ...
            logger.info(f"[CodeRetriever] Found code for {qualified_name} at {target_file}:{info['line_start']}")
...
def create_code_retrieval_tool(code_retriever: CodeRetriever) -> Tool:
    ...
    async def get_code_snippet(ctx: RunContext, 
                              qualified_name: str) -> CodeSnippet:
        """
