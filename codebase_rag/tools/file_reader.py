...
class FileReader:
    def __init__(self, project_root: str = "."):
        ...
        logger.info(logs.FILE_READER_INIT.format(root=self.project_root))

    async def read_file(self, file_path: str) -> FileReadResult:
        logger.info(logs.TOOL_FILE_READ.format(path=file_path))
        try:
            ...
            try:
                ...
                return FileReadResult(
                    file_path=file_path,
                    error_message=logs.FILE_OUTSIDE_ROOT.format(action="read"),
                )
            ...
            if full_path.suffix.lower() in self.binary_extensions:
                ...
                logger.warning(logs.TOOL_FILE_BINARY.format(message=error_msg))
                return FileReadResult(file_path=file_path, error_message=error_msg)

            try:
                content = full_path.read_text(encoding=ENCODING_UTF8)
                logger.info(logs.TOOL_FILE_READ_SUCCESS.format(path=file_path))
            ...
            logger.error(logs.FILE_READER_ERR.format(path=file_path, error=e))
            return FileReadResult(
                file_path=file_path,
                error_message=logs.UNEXPECTED.format(error=e),
            )
