from typing import TYPE_CHECKING, NamedTuple # NamedTuple imported to resolve linting failures, but not used. Removing.
    ...
    def resolve_tool_name(canonical: AgenticToolName) -> str | None:
        if canonical not in registered:
            logger.warning(
                ...
                "references to it in the orchestrator prompt will be omitted."
            )
            return None
...
def build_rag_orchestrator_prompt(
    ...
    # --- Fixed Introductory Rules ---
    prompt_lines.append("You are an expert AI assistant for analyzing codebases. Your answers are based **EXCLUSIVELY** on information retrieved using your tools.")
    prompt_lines.append("")
    prompt_lines.append("**CRITICAL RULES:**")
    prompt_lines.append("1.  **TOOL-ONLY ANSWERS**: You must ONLY use information from the tools provided. Do not use external knowledge.")

    # Rule 2: Natural Language Queries (conditional on query_graph tool)
    if t.query_graph:
        prompt_lines.append(f"2.  **NATURAL LANGUAGE QUERIES**: When using the `{t.query_graph}` tool, ALWAYS use natural language questions. NEVER write Cypher queries directly - the tool will translate your natural language into the appropriate database query.")
    else:
        # If query_graph is not available, Rule 2 is omitted, and subsequent rules will renumber.
        pass

    # Rule 3: Honesty (renumbered dynamically if Rule 2 was omitted, implicitly handled by following `idx` below)
    prompt_lines.append("3.  **HONESTY**: If a tool fails or returns no results, you MUST state that clearly and report any error messages. Do not invent answers.")

    # Rule 4: Choose the Right Tool for the File Type (conditional on read_file tool)
    file_type_rules_content = []
    if t.read_file:
        file_type_rules_content.append(f"- For source code files (.py, .ts, etc.), use `{t.read_file}`.")
    file_type_rules_content.append("- Images and PDFs the user references are attached inline to the message; read them directly from your own multimodal input.")

    if file_type_rules_content:
        prompt_lines.append("4.  **CHOOSE THE RIGHT TOOL FOR THE FILE TYPE**:")
        prompt_lines.extend(file_type_rules_content)

    prompt_lines.append("") # Ensure a blank line before "Your General Approach"
    prompt_lines.append("**Your General Approach:**")

    # --- Dynamic 'Your General Approach' Sections ---
    general_approach_sections: list[list[str]] = []

    # Section 1: Inspect Attached Media Directly
    general_approach_sections.append([
        "Inspect Attached Media Directly: When the user attaches an image or PDF, analyze it from the inline content of the message. Do not call a tool for it."
    ])

    # Section 2: Deep Dive into Code
    deep_dive_sub_items = [
        "First, check if documentation files like `README.md` exist and read them for context. For configuration, look for files appropriate to the language (e.g., `pyproject.toml` for Python, `package.json` for Node.js).",
        "**Then, you MUST dive into the source code.** Explore the `src` directory (or equivalent). Identify and read key files (e.g., `main.py`, `index.ts`, `app.ts`) to understand the implementation details, logic, and functionality.",
        "Synthesize all this information—from documentation, configuration, and the code itself—to provide a comprehensive, factual answer. Do not just describe the files; explain what the code *does*.",
        "Only ask for clarification if, after a thorough investigation, the user's intent is still unclear.",
    ]
    formatted_deep_dive_sub_items = [f"    {chr(ord('a') + i)}. {item}" for i, item in enumerate(deep_dive_sub_items)]
    general_approach_sections.append([
        "Deep Dive into Code: When you identify a relevant component (e.g., a folder), you must go beyond documentation.",
        *formatted_deep_dive_sub_items
    ])

    # Section 3: Choose the Right Search Strategy - SEMANTIC FIRST for Intent
    search_strategy_sub_sections: list[str] = []
    current_sub_letter = ord('a')

    if t.semantic_search:
        semantic_first_block = [
        ...
        search_strategy_sub_sections.append(f"{chr(current_sub_letter)}. " + "\n        ".join(semantic_first_block))
        current_sub_letter += 1

    if t.query_graph:
        graph_directly_block = [
    ...
    if t.semantic_search and t.query_graph and t.read_file:
        ...
        search_strategy_sub_sections.append(f"{chr(current_sub_letter)}. " + "\n        ".join(tool_chaining_block))
        # current_sub_letter += 1 # No need to increment after the last item

    if search_strategy_sub_sections:
        general_approach_sections.append([
            "Choose the Right Search Strategy - SEMANTIC FIRST for Intent:",
            *search_strategy_sub_sections
        ])

    # Section 4: Plan Before Writing or Modifying (lines 267-283 in original)
    plan_sub_items = []
    tool_list = []
    if t.create_file:
        tool_list.append(f"`{t.create_file}`")
    if t.edit_file:
        tool_list.append(f"`{t.edit_file}`")

    if tool_list:
        tools_str = ""
        if len(tool_list) == 1:
            tools_str = tool_list[0]
        elif len(tool_list) == 2:
            tools_str = f"{tool_list[0]} or {tool_list[1]}"
        else: # Unlikely with only two edit tools, but general case
            tools_str = ", ".join(tool_list[:-1]) + f", or {tool_list[-1]}"
        plan_sub_items.append(f"Before using {tools_str}, or modifying files, you MUST explore the codebase to find the correct location and file structure.")

    if t.shell_command:
        plan_sub_items.append(f"For shell commands: If `{t.shell_command}` returns a confirmation message (return code -2), immediately return that exact message to the user. When they respond \"yes\", call the tool again with `user_confirmed=True`.")

    if plan_sub_items:
        formatted_plan_sub_items = [f"    {chr(ord('a') + i)}. {item}" for i, item in enumerate(plan_sub_items)]
        general_approach_sections.append([
            "Plan Before Writing or Modifying:",
            *formatted_plan_sub_items
        ])

    # Section 5: Execute Shell Commands (lines 284-286 in original)
    if t.shell_command:
        general_approach_sections.append([
    ...
    # Section 6: Complete the Investigation Cycle
    investigation_cycle_sub_items = []
    if t.semantic_search and t.query_graph and t.read_file: # All three tools typically used here
        investigation_cycle_sub_items = [
            "Find candidate functions via semantic search",
            "Explore their relationships via graph queries",
            "**AUTOMATICALLY read main.py** (or main entry file) - NEVER ask the user for permission",
            "Look for the ACTUAL startup code: `if __name__ == \"__main__\"`, CLI commands, `main()` functions",
            "If CLI framework detected (typer, click, argparse), examine command functions",
            "Distinguish between helper functions and the real application entry point",
            "Show the complete execution flow from the true entry point through initialization",
        ]
    if investigation_cycle_sub_items:
        formatted_investigation_sub_items = [f"    {chr(ord('a') + i)}. {item}" for i, item in enumerate(investigation_cycle_sub_items)]
        general_approach_sections.append([
            "Complete the Investigation Cycle: For entry point queries, you MUST:",
            *formatted_investigation_sub_items
        ])

    # Section 7: Token Management
    token_management_sub_items = []
    if t.semantic_search or t.read_file: # Relevant if any retrieval tools are present
        token_management_sub_items = [
            "For semantic search, use focused queries (not overly broad terms)",
            "For file reading, read specific sections when possible using offset/limit",
            "Summarize large results rather than including full content",
            "Prioritize most relevant findings over comprehensive coverage",
        ]
    if token_management_sub_items:
        formatted_token_management_sub_items = [f"    {chr(ord('a') + i)}. {item}" for i, item in enumerate(token_management_sub_items)]
        general_approach_sections.append([
            "Token Management: Be efficient with context usage:",
            *formatted_token_management_sub_items
        ])

    # Section 8: Synthesize Answer
    general_approach_sections.append([
        "Synthesize Answer: Analyze and explain the retrieved content. Cite your sources (file paths or qualified names). Report any errors gracefully."
    ])


    # Join the dynamically generated general approach sections with correct numbering
    for idx, section_lines in enumerate(general_approach_sections):
        main_title = section_lines[0]
        # Check if the title already contains a specific prefix (e.g., "Deep Dive into Code:")
        # or if it's just a bold title like "**Synthesize Answer**".
        if ':' in main_title and not main_title.startswith("**"):
            title_part, rest = main_title.split(':', 1)
            prompt_lines.append(f"{idx + 1}. {title_part.strip()}: {rest.strip()}")
        else:
            prompt_lines.append(f"{idx + 1}. {main_title.strip()}")

        for line in section_lines[1:]:
            prompt_lines.append(line)
        if idx < len(general_approach_sections) - 1: # Add blank line between sections
            prompt_lines.append("")
