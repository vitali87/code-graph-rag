...
class MCPToolsRegistry:
    ...
    def _run_rename(
        ...
        try:
            ...
            # `_reingest_for_contract` routes through
            # `_updater_for_reingest`, which rejects a cached updater
            # built for a different project, so the delta is measured
            # under the same name the re-ingest writes.
            reingest = self._reingest_for_contract(project_name)
            report = rename(
                root,
                self.ingestor.fetch_all,
