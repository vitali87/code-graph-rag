...
class JavaTypeInferenceEngine(
    ...
    def _build_fqn_lookup_map(self) -> dict[str, list[str]]:
        ...
        for module_qn in self.module_qn_to_file_path.keys():
            ...
            # (H) Without a recognized src/main/java layout find_package_start_index
            # (H) returns None, leaving the whole map empty so cross-file Java
            # (H) resolution (static calls, instance dispatch in sibling files)
            # (H) silently fails. Fall back to the segment after the project root
            # (H) (index 1) so flat / non-standard layouts still register their
            # (H) simple class names. find_package_start_index never returns 0.
